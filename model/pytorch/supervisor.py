...
class GTSSupervisor:
    def __init__(self, save_adj_name, temperature, **kwargs):
        ...
        elif self._data_kwargs['dataset_dir']=='data/CHARGE':
            df= pd.read_hdf('./data/New_hdf5_charge_final.h5')
        elif self._data_kwargs['dataset_dir']=='data/CALCE_2':
            df= pd.read_hdf('./data/calce_2.h5')
        elif self._data_kwargs['dataset_dir']=='data/SMARTPHONE':
            df=pd.read_hdf('./data/smartphone.h5')
        elif self._data_kwargs['dataset_dir']=='data/SMARTPHONE_18_09':
            df=pd.read_hdf('./data/smartphone_18_09.h5')
        elif self._data_kwargs['dataset_dir']=='data/US06_11_11_50':
            df=pd.read_hdf('./data/us06_11_11_50.h5')
        elif self._data_kwargs['dataset_dir']=='data/1_RAWDATA':
            df=pd.read_hdf('./data/1_rawdata.h5')
        elif self._data_kwargs['dataset_dir']=='data/2_RAWDATA':
            df=pd.read_hdf('./data/2_rawdata.h5')
        elif self._data_kwargs['dataset_dir']=='data/3_RAWDATA':
            df=pd.read_hdf('./data/3_rawdata.h5')
        elif self._data_kwargs['dataset_dir']=='data/4_RAWDATA':
            df=pd.read_hdf('./data/4_rawdata.h5')
        elif self._data_kwargs['dataset_dir']=='data/5_RAWDATA':
            df=pd.read_hdf('./data/5_rawdata.h5')
        #else:
        ...
        self.seq_len = int(self._model_kwargs.get('seq_len', 1))  # for the encoder
        ...
        with torch.no_grad():
            ...
            for batch_idx, (x, y) in enumerate(val_iterator):
                ...
                if label == 'without_regularization': 
                    ...
                    #plt.savefig('without regularization.jpg')
                    ...
                    if batch_idx==0:
                        ...
                        #y_true_df=pd.DataFrame(y_true.numpy())
                        torch.save(y_pred,'y_pred.pt')
                        torch.save(y_true,'y_true.pt')
                        '''
                        plt.plot(y_pred_df.index,y_pred[1])
                        plt.plot(y_true_df.index,y_true[1])
                        plt.savefig('without regularization.jpg')
                        '''
                #if batch_idx % 100 == 1:
